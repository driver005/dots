...
#   1: helix (hx)  2: claude  3: shell  4: opencode  5: todos (tdo)
...
if ! tmux has-session -t "$session" 2>/dev/null; then
    tmux new-session -d -s "$session" -c "$path" -n "helix"    "$(keep 'hx')"
    ...
    tmux new-window  -t "$session" -c "$path" -n "todos"    "$(keep 'EDITOR=hx NOTES_DIR="$HOME/notes" tdo')"
